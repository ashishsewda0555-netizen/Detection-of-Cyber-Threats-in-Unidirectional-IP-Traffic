...
function AuxiliaryView({ activeSection, setActiveSection, connected, packets, threats, health }: { activeSection: string; setActiveSection: (section: string) => void; connected: boolean; packets: TrafficPacket[]; threats: TrafficPacket[]; health: HealthStatus | null }) {
  ...
  const featureData = latestThreatWithEvidence?.evidence
    ...
    : [["Packet size", "High", 86], ["Target port", "Elevated", 74], ["Payload entropy", "Moderate", 61], ["Protocol", "Stable", 42]] as [string, string, number][];

  return <div className="aux-shell"><aside className="aux-rail"><div className="brand"><img src="/manus-storage/signal-room-sentinel_222b9f60.png" alt="Signal Room sentinel" /><div><strong>signal<span>/</span>room</strong><small>THREAT SENSING GRID</small></div></div><div className="rail-label">WORKSPACE</div><nav>{["Overview", "Traffic monitor", "Threat intel", "Model health"].map(name => <button key={name} className={activeSection === name ? "nav-active" : ""} onClick={() => setActiveSection(name)}><Activity size={16} /><span>{name}</span>{name === "Threat intel" && <b>{String(threats.length).padStart(2, "0")}</b>}</button>)}</nav><div className="aux-rail-foot"><span className="status-dot" /> {connected ? "ML PIPELINE CONNECTED" : "DEMO FALLBACK"}</div></aside><main className="aux-main"><div className="eyebrow"><span className="live-pip" /> LIVE OPERATIONS / {activeSection.toUpperCase()}</div><div className="aux-title-row"><div><h1>{copy.title}</h1><p>{copy.intro}</p></div><div className={`connection-pill ${connected ? "is-connected" : "is-offline"}`}><Wifi size={14} /> {connected ? "Pipeline active" : "Demo mode"}</div></div>{activeSection === "Traffic monitor" && <section className="aux-grid"><div className="panel aux-hero-card"><span className="section-kicker">{copy.kicker}</span><h2>One-way flow is <em>stable.</em></h2><div className="flow-visual"><div className="flow-node"><span className="status-dot" /><strong>INGRESS</strong><small>{connected ? "LIVE CAPTURE" : "PACKET REPLAY"}</small></div><div className="flow-line"><i /><i /><i /><i /><i /></div><div className="flow-node output"><span className="status-dot" /><strong>ANALYSIS</strong><small>{connected ? "DUAL ENGINE v2.0" : "MODEL v1.1.0"}</small></div></div><div className="aux-stats"><div><span>WINDOWS SCORED</span><strong>{totalWindows || 18421}</strong></div><div><span>TCP</span><strong>{protocolCounts.TCP || 64}%</strong></div><div><span>UDP</span><strong>{protocolCounts.UDP || 24}%</strong></div><div><span>ICMP</span><strong>{protocolCounts.ICMP || 12}%</strong></div></div></div><div className="panel aux-list-card"><span className="section-kicker">RECENT TELEMETRY</span><h2>Stream checkpoints</h2>{(packets.length ? packets.slice(-5).reverse() : seedThreats).map((packet, i) => <div className="checkpoint-row" key={`${packet.timestamp}-${i}`}><span className="status-dot" /><div><strong>{packet.protocol} / port {packet.target_port}</strong><small>{packet.packet_size ? `${packet.packet_size} bytes` : `anomaly ${(packet.anomaly_score ?? packet.payload_entropy).toFixed(2)}`} · entropy {packet.payload_entropy.toFixed(2)}</small></div><time>{formatTime(packet.timestamp)}</time></div>)}</div></section>}{activeSection === "Threat intel" && <section className="panel intelligence-board"><div className="panel-heading"><div><span className="section-kicker">{copy.kicker}</span><h2>Attack signatures {connected ? "detected" : "in replay"}</h2></div><span className="count-badge">{String(threats.length).padStart(2, "0")}</span></div><div className="intel-cards"><div className="intel-card dos"><span className="attack-badge dos">VOLUMETRIC DOS</span><strong>{threats.filter(t => t.threat_type === "Volumetric DoS" || t.threat_type === "DDoS spike" || t.threat_type === "SYN Flood DDoS").length}</strong><small>high-volume service exhaustion pattern</small></div><div className="intel-card recon"><span className="attack-badge recon">RECONNAISSANCE</span><strong>{threats.filter(t => t.threat_type === "Port scan" || t.threat_type === "Port Scan").length + portScanCount}</strong><small>sequential target-port discovery pattern</small></div><div className="intel-card"><span className="attack-badge neutral">{connected ? "UNKNOWN VARIANT" : "MALICIOUS PAYLOAD"}</span><strong>{threats.filter(t => t.threat_type === "Malicious payload" || t.threat_type === "Unknown DDoS Variant").length}</strong><small>{connected ? "anomaly-flagged unclassified pattern" : "high-entropy payload anomaly"}</small></div></div><div className="intel-table">{threats.map((threat, index) => <button key={`${threat.timestamp}-${index}`} onClick={() => setActiveSection("Threat intel")}><span className={`type-marker ${threat.severity}`} /><strong>{threat.threat_type}</strong><span className="mono">{threat.source_ip}</span><b>{threat.confidence}%</b></button>)}</div></section>}{activeSection === "Model health" && <section className="aux-grid model-health-grid"><div className="panel score-card"><span className="section-kicker">{copy.kicker}</span><h2>{connected ? "Dual-Engine posture" : "Random Forest posture"}</h2><div className="health-score"><strong>{modelConfidence}<sup>%</sup></strong><span>operational confidence</span></div><div className="health-meter"><span style={{ width: `${modelConfidence}%` }} /></div>{connected && stats ? <p>Dual-engine pipeline active. {totalWindows} windows scored: {criticalCount} critical, {highCount} high, {mediumCount} medium alerts. {benignCount} benign. {health?.sse_clients ?? 0} SSE client(s) connected.</p> : <p>Feature inputs are arriving within expected ranges. The playback engine is supplying a stable, labeled stream for demo inference.</p>}</div><div className="panel feature-card"><span className="section-kicker">FEATURE CONTRIBUTION</span><h2>Decision signals</h2>{featureData.map(([label, state, width]) => <div className="feature-row" key={label as string}><div><strong>{label}</strong><small>{state}</small></div><div className="progress-track"><span style={{ width: `${width}%` }} /></div></div>)}</div></section>}<footer className="footer"><span>© 2026 SIGNAL ROOM / SIH26145 PROTOTYPE</span><span><span className="status-dot" /> {connected ? `ML PIPELINE / ${health?.sse_clients ?? 0} CLIENTS` : "PLAYBACK ENGINE / 1.0s CADENCE"}</span></footer></main></div>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const testData = [
      {
        "flow_id": "192.168.100.2-window-2026-09-05 12:02:40.000000+00:00",
        "timestamp": "2026-09-05 12:02:40.000000+00:00",
        "src_ip": "single_source",
      ...
      {
        "flow_id": "192.168.100.2-window-2026-09-05 12:02:48.264707+00:00",
        "timestamp": "2026-09-05 12:02:48.264707+00:00",
        "src_ip": "Multiple/Spoofed (43210 IPs)",
        "dst_ip": "192.168.100.2",
        "anomaly_score": 0.868,
        ...
        "severity": "critical",
        "evidence": {
          "unique_src_count": 43210,
          "src_ip_entropy": 0.0,
          "syn_flag_sum": 43213
        }
      },
      {
        "flow_id": "192.168.100.2-window-2026-09-05 12:03:36.472707+00:00",
  ...
  const navItems = useMemo(() => [{ name: "Overview", icon: Activity }, { name: "Traffic monitor", icon: Radio }, { name: "Threat intel", icon: ShieldAlert }, { name: "Model health", icon: Cpu }], []);
  ...
  return <div className="soc-shell">
    ...
    <main className="main-content">
      ...
      <section className="kpi-grid"><div className="kpi-card"><div className="kpi-top"><span>WINDOWS ANALYZED</span><Database size={16} /></div><strong>{formatNumber(total)}</strong><div className="kpi-foot"><span className="positive">+{((packets.length / Math.max(total, 1)) * 100).toFixed(1)}%</span><span>{streamMode === "sse" ? "ML pipeline windows" : "playback records"}</span></div></div><div className="kpi-card"><div className="kpi-top"><span>AVG. AI CONFIDENCE</span><Gauge size={16} /></div><strong>{avgConfidence}<sup>%</sup></strong><div className="kpi-foot"><span className="positive">+2.4%</span><span>model certainty</span></div></div><div className="kpi-card model-card"><div className="kpi-top"><span>MODEL STATUS</span><CircleDot size={16} /></div><strong className="model-live">{health?.model_loaded || streamMode === "demo" ? "READY" : "LOADING"}</strong><div className="kpi-foot"><span>{streamMode === "sse" ? "Dual Engine" : "Random Forest"}</span><span>{streamMode === "sse" ? "IsoForest + RF + SHAP" : "5 features / v1.1.0"}</span></div></div></section>
