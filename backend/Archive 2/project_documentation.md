...
5. [Pipeline Stages — End-to-End Walkthrough](#5-pipeline-stages--end-to-end-walkthrough)
   ...
   - [Stage 9: API Backend & SSE Real-Time Streaming](#stage-9-api-backend--sse-real-time-streaming)
...
13. [Phase 2 Updates: Pipeline Rewrite & Data Contract Fixes](#13-phase-2-updates-pipeline-rewrite--data-contract-fixes)
...
The pipeline starts from raw `.pcap` packet captures and ends at a **live React dashboard** connected to a FastAPI backend via Server-Sent Events (SSE) — delivering real-time, in-memory threat verdicts with zero disk I/O in the inference path.
...
> [!IMPORTANT]
> **No CSV files exist in the live inference path.** The Scapy sniffer extracts features in-memory and POSTs a JSON payload directly to the FastAPI backend. The backend scores the window in-memory via `scorer.py` and broadcasts the structured verdict to all connected React frontends via Server-Sent Events (SSE). This eliminates the race conditions, file-lock errors, and latency bottlenecks of the legacy disk-polling architecture.
...
After merging all 23 flow CSVs, the result is [ml_ready_data.csv](file:///f:/ddos_ml_pipeline/ml_ready_data.csv):
...
**Output:** 23 `.pcap` files in [pcap_training_inputs/](file:///f:/ddos_ml_pipeline/pcap_training_inputs)
...
**Input:** 23 `.pcap` files
**Output:** 23 `*_Flow.csv` files in [output_csv/](file:///f:/ddos_ml_pipeline/output_csv)
...
**Script:** [merge.py](file:///f:/ddos_ml_pipeline/merge.py)
**Input:** 23 CSV files from `output_csv/`
**Output:** [ml_ready_data.csv](file:///f:/ddos_ml_pipeline/ml_ready_data.csv) (~960 MB)
...
**Script:** [00 infer labels.py](file:///f:/ddos_ml_pipeline/00%20infer%20labels.py)
**Input:** `ml_ready_data.csv`
**Output:** [label_mapping_suggested.csv](file:///f:/ddos_ml_pipeline/label_mapping_suggested.csv)
...
**Script:** [build_final_label_mapping.py](file:///f:/ddos_ml_pipeline/build_final_label_mapping.py)
**Input:** Human review of `label_mapping_suggested.csv`
**Output:** [label_mapping_final.csv](file:///f:/ddos_ml_pipeline/label_mapping_final.csv)
...
**Script:** [01_build_windowed_features.py](file:///f:/ddos_ml_pipeline/01_build_windowed_features.py)
**Input:** `ml_ready_data.csv` + `label_mapping_final.csv`
**Output:** [windowed_features.csv](file:///f:/ddos_ml_pipeline/windowed_features.csv) (~10 MB)
...
**Script:** [02 train ddos model.py](file:///f:/ddos_ml_pipeline/02%20train%20ddos%20model.py)
**Input:** `windowed_features.csv`
**Output:** [ddos_dual_engine_model.joblib](file:///f:/ddos_ml_pipeline/ddos_dual_engine_model.joblib) (~3.8 MB)
...
**Script:** [scorer.py](file:///f:/ddos_ml_pipeline/scorer.py)
...
**Script:** [live_capture.py](file:///f:/ddos_ml_pipeline/live_capture.py)

This script uses **Scapy** to sniff live network traffic in 5-second windows and extract the same features the model expects. All features are computed in-memory and immediately POSTed as a JSON payload to the FastAPI backend — **no CSV files are written to disk**.
...
def send_to_api(features):
    try:
        requests.post("http://localhost:8000/api/score", json=features, timeout=2)
...
while True:
    ...
    del packets  # Free memory immediately
...
| `unique_dst_ports` | Count of distinct destination TCP ports (for port scan detection) |

Each captured window is **POSTed as JSON** to `POST /api/score` for immediate in-memory scoring.

> [!NOTE]
> The sniffer performs a health check against `GET /api/health` on startup to confirm the FastAPI backend is reachable. If the API is offline, the sniffer logs a warning and continues attempting to POST each window — the capture loop is resilient to temporary API outages.

---

### Stage 9: API Backend & SSE Real-Time Streaming

**Script:** [api_server.py](file:///f:/ddos_ml_pipeline/api_server.py)

The FastAPI backend is the central nervous system of the live inference pipeline. It replaces the legacy Streamlit dashboard entirely.

#### Endpoints

| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/score` | Receives a 5-second feature window as JSON, scores it via `scorer.py`, broadcasts verdict to all SSE clients |
| `GET` | `/api/stream` | SSE endpoint — React frontend opens a persistent `EventSource` connection here and receives pushed JSON verdicts in real-time |
| `GET` | `/api/health` | Health check — confirms model is loaded, returns connected client count and cumulative stats |
| `GET` | `/api/history` | Returns the last 50 scored verdicts (for late-joining clients) |

#### SSE Client Manager

The backend maintains an `SSEClientManager` that uses `asyncio.Queue` fan-out to broadcast verdicts to all connected frontends:

```python
# When a new verdict is scored:
await sse_manager.broadcast(verdict)  # Pushes to every connected client's queue

# React frontend connects via EventSource:
...
- Each connected client gets its own `asyncio.Queue` (max 256 items)
- A rolling buffer of the last 50 verdicts is maintained for late-joining clients
- 15-second keepalive comments prevent connection timeouts
- Slow clients are handled gracefully (oldest items dropped, not blocked)

#### CORS Configuration

> [!IMPORTANT]
> Because the React frontend (port 3000 or 5173) and the FastAPI backend (port 8000) run on different ports, the browser treats them as **different origins**. `CORSMiddleware` is configured to whitelist the React frontend's origin, preventing the browser from blocking the SSE stream due to Cross-Origin Resource Sharing policies.
...
Zero disk I/O in the entire live inference path. All data flows through in-memory structures.
...
**Script:** [inject_zeroday.py](file:///f:/ddos_ml_pipeline/inject_zeroday.py)
...
| Step | File | Purpose |
|---|---|---|
| Pre-req | [merge.py](file:///f:/ddos_ml_pipeline/merge.py) | Concatenate all CICFlowMeter CSVs, tag with `source_file` |
| Step 0 | [00 infer labels.py](file:///f:/ddos_ml_pipeline/00%20infer%20labels.py) | Heuristic label inference from traffic characteristics |
| Step 0.5 | [build_final_label_mapping.py](file:///f:/ddos_ml_pipeline/build_final_label_mapping.py) | Encode the human-reviewed labels into `label_mapping_final.csv` |
| Step 1 | [01_build_windowed_features.py](file:///f:/ddos_ml_pipeline/01_build_windowed_features.py) | Fixed-size windowed feature engineering |
| Step 2 | [02 train ddos model.py](file:///f:/ddos_ml_pipeline/02%20train%20ddos%20model.py) | Train dual-engine model, evaluate, save |

### Inference & Deployment

| File | Purpose |
|---|---|
| [scorer.py](file:///f:/ddos_ml_pipeline/scorer.py) | Production inference module — loads model, exposes `score_window()` |
| [api_server.py](file:///f:/ddos_ml_pipeline/api_server.py) | FastAPI backend — receives scored windows via POST, broadcasts verdicts via SSE |
| [live_capture.py](file:///f:/ddos_ml_pipeline/live_capture.py) | Real-time Scapy packet sniffer — POSTs feature vectors as JSON to the API (no disk I/O) |
| [.env](file:///f:/ddos_ml_pipeline/.env) | Centralized configuration — `VICTIM_IP`, `SNIFFER_INTERFACE`, `API_HOST`, `API_PORT` |

> [!WARNING]
> **`dashboard.py` has been deleted.** It was a legacy Streamlit dashboard that polled `live_stream.csv` on disk every 1 second — a fundamentally broken streaming mechanism with race conditions and file-lock errors. It has been replaced entirely by `api_server.py` + SSE + React.

### Utilities & Diagnostics

| File | Purpose |
|---|---|
| [inject_zeroday.py](file:///f:/ddos_ml_pipeline/inject_zeroday.py) | Inject synthetic zero-day anomaly vectors for testing |
| [capture_log.txt](file:///f:/ddos_ml_pipeline/Archive%202/capture_log.txt) | Mandatory capture logging — timestamps, IPs, labels for every PCAP |
| [run_windowing.py](file:///f:/ddos_ml_pipeline/run_windowing.py) | Early prototype windowing script (superseded by `01_build_windowed_features.py`) |
| [label.py](file:///f:/ddos_ml_pipeline/label.py) | Early prototype threshold-based labeling (superseded) |
| [diag.py](file:///f:/ddos_ml_pipeline/diag.py) | Diagnostic script to inspect time spans and destination IP diversity |
...
| `ddos_dual_engine_model.joblib` | ~3.8 MB | Serialized dual-engine model bundle |
...
```bash
pip install pandas numpy scikit-learn shap joblib scapy fastapi uvicorn python-dotenv requests
```

### Configuration

All environment-specific settings are in a single `.env` file (not committed to git):
...
### Live Inference (Three Terminals)

```bash
# Terminal 1: Start the FastAPI backend (loads model on startup)
python api_server.py
# Confirm: http://localhost:8000/api/health should return {"status": "ok"}

# Terminal 2: Start the Scapy sniffer (requires admin/root for raw sockets)
sudo python live_capture.py
# Sniffer will auto-detect the API and begin POSTing feature windows

# Terminal 3: Start the React frontend
cd frontend && npm run dev
# Open http://localhost:3000 (or :5173 for Vite)
# The UI connects to http://localhost:8000/api/stream via EventSource
```
...
## 13. Phase 2 Updates: Pipeline Rewrite & Data Contract Fixes

In Phase 2, the system was heavily refined to fix train/serve skew and introduce parallel heuristic detection.

### 1. Train/Serve Skew Fixed (5-Second Time Buckets)
**Problem:** The original training pipeline aggregated traffic in arbitrary chunks of 30 rows. However, the production `live_capture.py` sniffer processed traffic strictly in 5-second wall-clock intervals. This discrepancy (train/serve skew) severely undermined model reliability in real-world scenarios.
**Solution:** `01_build_windowed_features.py` was rewritten to group rows by exactly 5-second intervals using Pandas' `pd.Grouper(freq='5s')`. The calculation for `flow_rate` and `packet_rate` was unified to precisely `(total_packets / 5.0)` across training and inference, perfectly aligning the model's training distribution with real-time reality.

### 2. Parallel Port Scan Detection
**Problem:** Port scans are deterministic and fundamentally different from volumetric DDoS attacks, yet they were not natively handled by the ML engines.
**Solution:** The `unique_dst_ports` feature was extracted and integrated. A parallel rule was added directly to `scorer.py`: if `unique_dst_ports >= 20` within a 5-second window, the system bypasses the ML engines entirely and immediately fires a `high` severity `port_scan` alert with `confidence=1.0`.

### 3. Spoofed Source IP Learning
**Problem:** The model lacked authentic training examples of distributed/spoofed IPs, meaning it could not adequately recognize the variance that defines a spoofed SYN flood.
**Solution:** A 450 MB `spoofed_flood_01.pcap` generated via `hping3 --rand-source` was converted using a new `pcap_to_features.py` script. The Random Forest was retrained on this data, successfully learning the elevated `unique_src_count` and `src_ip_entropy` characteristics.

### 4. API Data Contract Alignment
**Problem:** The UI/UX dashboard team required a very specific JSON schema that was not previously matched by the ML output.
**Solution:** The inference contract in `scorer.py` was structurally overhauled:
- Renamed keys: `confidence_score` → `confidence`, `supporting_evidence` → `evidence`.
- Injected required context: `timestamp`, `src_ip`, and `dst_ip`.
- Auto-generated a deterministic Flow ID: `f"{dst_ip}-window-{window_start_ts}"`.

---

## 14. Phase 3 Updates: Architecture Reform (Disk I/O Elimination)

Phase 3 was a fundamental architectural overhaul that replaced the disk-bound inference pipeline with a zero-I/O, API-driven streaming architecture.

### 1. CSV Eliminated from Live Path
**Problem:** The legacy architecture used `live_stream.csv` as a data-passing mechanism between the Scapy sniffer and the Streamlit dashboard. Two separate Python processes accessing the same file simultaneously guaranteed race conditions, file-lock errors, and dropped frames. Disk I/O introduced massive unnecessary latency, choking a high-speed anomaly detector through a physical disk bottleneck.
**Solution:** `live_stream.csv` has been deleted. `live_capture.py` now POSTs feature vectors as JSON directly to the FastAPI backend's `POST /api/score` endpoint. All data flows through in-memory structures.

### 2. Streamlit Dashboard Replaced with FastAPI + SSE
**Problem:** `dashboard.py` polled a CSV file every 1 second — a fundamentally broken streaming mechanism.
**Solution:** `dashboard.py` has been deleted entirely. The FastAPI backend (`api_server.py`) uses Server-Sent Events (SSE) to push scored verdicts to the React frontend via a persistent `EventSource` connection. An `SSEClientManager` uses `asyncio.Queue` fan-out to broadcast verdicts to all connected clients simultaneously.

### 3. Centralized Configuration via `.env`
**Problem:** `VICTIM_IP` and `SNIFFER_INTERFACE` were hardcoded in Python scripts, requiring code edits to change testing environments.
**Solution:** All environment-specific variables are extracted into a `.env` file loaded via `python-dotenv`. Both `live_capture.py` and `api_server.py` read from the same `.env` — a single source of truth.

### 4. CORS Middleware for Cross-Origin SSE
**Problem:** The React frontend (port 3000/5173) and FastAPI backend (port 8000) run on different ports, causing the browser to block SSE connections due to Cross-Origin Resource Sharing (CORS) policies.
**Solution:** `CORSMiddleware` is configured in `api_server.py` to whitelist the React frontend's origin.

### 5. Asynchronous HTTP Dispatch (Fire-and-Forget)
**Problem:** In `live_capture.py`, `requests.post()` blocked the main thread synchronously. During high-volume DDoS floods, waiting for the HTTP response caused Scapy to drop packets, mathematically skewing the `packet_rate` ML feature.
**Solution:** The HTTP POST request was refactored into a background thread using Python's `threading` module. Scapy now dispatches the payload to the API asynchronously and immediately loops back to capture the next 5-second window, guaranteeing zero dropped packets.

---

## 15. Capture-Logging Discipline

> [!CAUTION]
> **Process failure that must never be repeated:** The original label mapping for the 22 Day-1 PCAPs had to be reverse-engineered via traffic heuristics because the pcap-to-attack-type mapping was lost at capture time. This is documented in Stage 3 (Automated Label Inference) and was a significant source of uncertainty.

The [capture_log.txt](file:///f:/ddos_ml_pipeline/Archive%202/capture_log.txt) file was created to prevent this failure from recurring. **Every PCAP captured for training must be logged with:**

| Field | Example | Purpose |
|---|---|---|
| `CAPTURE_ID` | `capture_023_spoofed_flood` | Unique identifier |
| `DATE` | `2026-09-05` | When it was captured |
| `START_TIME` / `STOP_TIME` | `02:19:58` / `02:20:47` | Exact wall-clock times |
| `ATTACKER_IP` | `RANDOMIZED` or `10.0.0.5` | Source IP(s) used |
| `VICTIM_IP` | `192.168.100.2` | Target machine |
| `ATTACK_TYPE` | `SPOOFED_SYN_FLOOD` | Ground-truth label |
| `TOOL_USED` | `hping3` | Exact tool |
| `COMMAND_USED` | `sudo hping3 --rand-source -S -p 80 --flood 192.168.100.2` | Exact command |
| `OUTPUT_FILE` | `spoofed_flood_01.pcap` | Exact output filename |
| `LABEL_APPLIED` | `ddos_spoofed_syn_flood` | Label used in training CSV |

This discipline is already applied for Day-1 forward captures (spoofed flood, deep benign). It **must** be applied for all port-scan and spoofed-source recaptures going forward.

---

## 16. Technical Differentiator: Dual-Engine + SHAP/Z-Score Explainability

The dual-engine architecture combined with dual explainability (TreeSHAP + z-score analysis) is the system's **primary technical differentiator** and a materially stronger answer to "confidence scores and supporting evidence" requirements than threshold-based alerting.

### Why This Matters

Most network security ML systems use a single model (usually a classifier) with a simple confidence threshold. This approach has two critical blindspots:

1. **Zero-day attacks are invisible.** A classifier can only predict classes it was trained on. A novel attack vector gets confidently misclassified as the nearest known class — a dangerous false negative.
2. **"The model said so" is not evidence.** A raw probability score (e.g., `0.94`) tells a SOC analyst *nothing* about *why* traffic was flagged.

### How We Solve Both

| Capability | Our Approach | Typical Approach |
|---|---|---|
| **Known attack detection** | Random Forest (200 trees) with calibrated class probabilities | Same (single classifier) |
| **Zero-day detection** | Isolation Forest trained exclusively on benign traffic — flags anything outside the "normal envelope" | ❌ Not present |
| **Evidence for known attacks** | TreeSHAP — exact per-feature contribution to the predicted class (e.g., "SYN/ACK ratio contributed +0.38 to SYN flood classification") | ❌ Raw probability only |
| **Evidence for zero-day alerts** | Z-score analysis against benign baseline (e.g., "unique source IPs are 48σ above normal") | ❌ Not present |
| **Severity grading** | 4-level matrix based on engine agreement (critical / high / medium / benign) | Binary (alert / no alert) |

### The Pitch

The dual-engine + dual-explainability design should be prominently featured on the **Innovation slide** of any presentation, not just the Technical Approach slide. It directly addresses:

- **"How do you detect attacks you've never seen before?"** → The Isolation Forest zero-day pathway with z-score evidence.
- **"How confident is your system, and can you show your work?"** → TreeSHAP feature contributions for every alert, not just a probability number.
- **"What happens when both engines disagree?"** → The severity matrix produces a nuanced response (medium vs. critical), not a binary flag.
